...
datos_accidentes.createOrReplaceTempView('Accidentes')

datos_accidentes.show()

result = spark.sql('''SELECT DISTINCT tipo_accidente, COUNT(tipo_accidente)OVER(PARTITION BY tipo_accidente) AS Cantidad FROM Accidentes ORDER BY Cantidad DESC''')
