...
def estacionamientos():
    ...
    datos_estacion= (spark.read.csv(datos[9],header=True, inferSchema=True, sep =";", encoding='Latin1'))

    datos_estacion.createOrReplaceTempView('estacionamientos')


    qt_plazas = spark.sql('''SELECT DISTINCT DISTRITO, SUM(`NUM PLAZAS`)OVER(PARTITION BY DISTRITO) AS QT_PLAZAS from estacionamientos WHERE MES = 1''')


    qt_plazas.toPandas().to_csv('output/qt_plazas.csv', index=None, sep=';')
